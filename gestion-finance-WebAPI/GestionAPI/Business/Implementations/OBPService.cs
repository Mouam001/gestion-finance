...
public class ObpService : IObpService
{
    ...
    public async Task<List<TransactionDto>> GetTransactionsAsync(string obpToken, string bankId, string accountId)
    {
        ...
        return transactions.Select(t =>
        {
            ...
            var amountStr = value?["amount"]?.ToString();
            var currency = value?["currency"]?.ToString() ?? "EUR";
            decimal.TryParse(amountStr, System.Globalization.NumberStyles.Any,
                System.Globalization.CultureInfo.InvariantCulture, out var amount);

            DateTime.TryParse(details?["posted"]?.ToString(), out var postedDate);
            DateTime.TryParse(details?["completed"]?.ToString(), out var completedDate);

            return new TransactionDto
            {
                ObpId = (string)t["id"],
                ObpType = details?["type"]?.ToString() ?? "Type inconnue",
                Description = details?["description"]?.ToString() ?? "Description inconnue",
                Amount = amount,
                ObpCurrency = currency,
                PostedDate = postedDate > new DateTime(1900, 1, 1) ? postedDate : null,
                CompletedDate = completedDate > new DateTime(1900, 1, 1) ? completedDate : null
            };
        }).ToList();
    }
